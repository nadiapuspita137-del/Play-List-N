...
create or replace view public.song_stats as select s.id,s.title,s.owner_id,count(*) filter(where e.event_type='play')::int plays,count(*) filter(where e.event_type='download')::int downloads from songs s left join song_events e on e.song_id::text=s.id::text group by s.id;
